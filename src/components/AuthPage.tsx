...
function getFriendlyAuthError(err: any): string {
  ...
  switch (code) {
    ...
    case 'auth/internal-error':
      return 'An unexpected authentication error occurred. Please refresh and retry.';
    default:
  ...
  return (
    <div className="min-h-screen bg-[#14110E] flex flex-col items-center justify-center p-6">
      ...
      <div
        ...
        style={{
          backgroundImage: 'radial-gradient(#E9BA6B 1px, transparent 1px)',
        ...
        <div className="bg-[#1F1A17] border border-[#3A3026] rounded-2xl p-8 shadow-2xl">
          ...
          {error && (
            <div
              role="alert"
              aria-live="assertive"
              className="mb-4 px-4 py-2.5 rounded-lg bg-rose-900/40 border border-rose-700/50 text-rose-300 text-xs font-mono text-center leading-relaxed"
            >
              {error}
            </div>
          )}
